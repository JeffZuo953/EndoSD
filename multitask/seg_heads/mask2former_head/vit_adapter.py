...
class ViTAdapter(TIMMVisionTransformer):
    ...
    def forward(self, x):
        h_s, w_s = (x.shape[2] - 1) // 2 + 1, (x.shape[3] - 1) // 2 + 1
        h_s, w_s = (h_s - 1) // 2 + 1, (w_s - 1) // 2 + 1

        deform_inputs1, deform_inputs2, spm_spatial_shapes = deform_inputs(x, self.patch_size)
        ...
        h_c2, w_c2 = spm_spatial_shapes[0]
        h_c3, w_c3 = spm_spatial_shapes[1]
        h_c4, w_c4 = spm_spatial_shapes[2]

        c2 = c2.transpose(1, 2).view(bs, dim, h_c2, w_c2).contiguous()
        c3 = c3.transpose(1, 2).view(bs, dim, h_c3, w_c3).contiguous()
        c4 = c4.transpose(1, 2).view(bs, dim, h_c4, w_c4).contiguous()
        c1 = c1.transpose(1, 2).view(bs, dim, h_s, w_s).contiguous()
        c1 = self.up(c2) + c1
