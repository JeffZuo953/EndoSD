...
# 解析命令行参数
CHECKPOINT_ROOT="/media/ExtHDD1/jianfu/data/train_lesion/multitask_dinov3_vits16plus_20250830_071130"
START_EPOCH=393
END_EPOCH=393
...
# 构建评估命令（多GPU模式）
# 注意：我们修改了逻辑，只创建一个tensorboard writer，所有epoch的评估都使用同一个writer
EVAL_CMD="${BASE_EVAL_CMD}"

# 添加checkpoint根目录和epoch范围作为参数
EVAL_CMD="${EVAL_CMD} --checkpoint-root \"${CHECKPOINT_ROOT}\" --start-epoch ${START_EPOCH} --end-epoch ${END_EPOCH}"
