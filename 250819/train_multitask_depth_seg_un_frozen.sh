...
NUM_CLASSES=3    # 分割类别数
MAX_DEPTH=0.2    # 最大深度值
SEG_INPUT_TYPE="last" # 分割头输入类型: "last", "last_four", "from_depth"
...
# 构建训练命令（DDP模式）
TRAIN_CMD="python -m torch.distributed.run --nproc_per_node=${NUM_GPUS} --master_port=20799 multitask/train_multitask_depth_seg.py \
    --encoder ${ENCODER} \
    --features ${FEATURES} \
