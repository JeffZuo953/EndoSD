...
# python evaluate_performance.py --encoder vits --img-size 518 --load-from /data/depthanything/depth_anything_v2_metric_hypersim_vits.pth --test-duration 60 --warmup-iters 100 --bs 100
...
def main():
    ...
    dummy_input = torch.randn(args.bs, 3, img_size[0], img_size[1]).cuda().float()
    ...
    logger.info(
        f"Starting performance test for {args.test_duration} seconds...")
...
# 
    while time.time() - test_start_time < args.test_duration:
        ...
        inference_time = start_event.elapsed_time(end_event)   # Calculate inference time
        inference_times.append(inference_time)  # Append to list
    ...
    if frame_count > 0:
        ...
        average_time_ms_forward = sum(inference_times) / frame_count
        average_fps = (1000.0 / average_time_ms)  # Convert ms per frame to frames per second
        average_fps_forward = (1000.0 / average_time_ms_forward)  # Convert ms per frame to frames per second
        ...
        logger.info(
            f"Test Duration:  ~{args.test_duration} seconds (actual GPU time: {total_duration_sec:.8f} s)"
        )
        logger.info(f"Warmup Iters:   {args.warmup_iters}")
        logger.info(f"Frames Processed: {frame_count}")
        logger.info(f"Average Time/Frame: {average_time_ms:.8f} ms")
        logger.info(
            f"Average Time/Frame (forward-only): {average_time_ms_forward:.8f} ms"
        )
        logger.info(f"Average FPS:        {average_fps:.6f}")
        logger.info(f"Average FPS:        {average_fps_forward:.6f}")
        logger.info(
            "=============================================================")
    else:
