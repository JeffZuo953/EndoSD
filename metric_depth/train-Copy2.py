...
def collate_fn_pad(batch):
    ...
    valid_masks = []
    image_paths = []
    max_depths = []  # 添加 max_depths 列表

    for item in batch:
        image = item['image']
        depth = item['depth']
        valid_mask = item['valid_mask']
        max_depth = item['max_depth']  # 获取 max_depth
        ...
        max_depths.append(max_depth)  # 添加 max_depth
    ...
    return {
        ...
        'max_depth': max_depths  # 添加 max_depth，作为列表返回
    }
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for i, sample in enumerate(trainloader):
            ...
            # 从列表中获取 max_depth 值，并转换为张量以进行广播
    ...
    # 1. 外层循环：从 DataLoader 获取一个批次的数据
    for i, sample in enumerate(valloader):
        img = sample["image"].cuda().float()
        depth = sample["depth"].cuda()
        valid_mask = sample["valid_mask"].cuda()
        max_depth_list = sample["max_depth"]  # 这是一个列表

        # 获取批次大小
        B = img.shape[0]

        with torch.no_grad():
            # 将 max_depth 列表转换为 [B, 1, 1] 的张量以进行广播
            max_depth_tensor = torch.tensor(max_depth_list,
                                            device=img.device).view(B, 1, 1)
        ...
        for j in range(B):
            ...
            depth_j = depth[j]
            valid_mask_j = valid_mask[j]
            current_max_depth_j = max_depth_list[j]

            # 为了公平比较, 将预测结果插值到与真值相同的尺寸
            # 注意: depth_j 此时是经过 collate_fn_pad 填充后的大小
            h, w = depth_j.shape
            pred_j_resized = F.interpolate(pred_j.unsqueeze(0).unsqueeze(0),
                                           (h, w),
                                           mode="bilinear",
                                           align_corners=True).squeeze()

            # (仅在 rank 0) 为第一个批次的第一个样本保存一张可视化图
            if rank == 0 and i == 0 and j == 0:
                ...
                pred_16bit = (pred_j_resized / current_max_depth_j * 65535).to(
                    torch.uint16)
                cv2.imwrite(output_path, pred_16bit.cpu().numpy())

            # 为第 j 个样本计算有效的评估掩码
            final_valid_mask_j = ((valid_mask_j == 1)
                                  & (depth_j >= args.min_depth)
                                  & (depth_j <= current_max_depth_j))
