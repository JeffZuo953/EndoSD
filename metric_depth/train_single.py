...
def main():
    ...
    if args.dataset == 'hypersim':
        ...
        trainset = C3VD('dataset/splits/c3vd/train.txt', 'train', size=size, max_depth=args.max_depth)
    else:
    ...
    if args.dataset == 'hypersim':
        ...
        valset = C3VD('dataset/splits/c3vd/val.txt', 'val', size=size, max_depth=args.max_depth)
    else:
        raise NotImplementedError
    ...
    optimizer = AdamW([
        {'params': [param for name, param in model.named_parameters() if ('pretrained' in name and param.requires_grad)], 'lr': args.lr},
        {'params': [param for name, param in model.named_parameters() if ('pretrained' not in name and param.requires_grad)], 'lr': args.lr * 10.0}
