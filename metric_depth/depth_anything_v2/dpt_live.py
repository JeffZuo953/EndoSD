...
def get_interpolate_frames(frame_list_pre, frame_list_post):
    ...
    post_w_list = [min_w
                        ...
                        for i in range(1,
                                       len(frame_list_pre) - 1)] + [max_w]
    interpolated_frames = []
    for i in range(len(frame_list_pre)):
        interpolated_frames.append(frame_list_pre[i] * (1 - post_w_list[i]) +
...
class DPTHead(nn.Module):
    ...
    def __init__(self,
        ...
        if use_clstoken:
            ...
            for _ in range(len(self.projects)):
                self.readout_projects.append(
                    nn.Sequential(nn.Linear(2 * in_channels, in_channels),
                                  nn.GELU()))
        ...
        self.scratch.output_conv1 = nn.Conv2d(head_features_1,
                                              head_features_1 // 2,
                                              kernel_size=3,
                                              stride=1,
                                              padding=1)
        self.scratch.output_conv2 = nn.Sequential(
            nn.Conv2d(head_features_1 // 2,
                      head_features_2,
                      kernel_size=3,
                      stride=1,
...
class DepthAnythingV2(nn.Module):
    ...
    def infer_video_depth_live(self,
                               frames,
                               input_size=(960, 540),
                               device='cuda',
                               fp32=False):
        ...
        append_frame_len = (frame_step -
                            (org_video_len % frame_step)) % frame_step + (
        ...
        for frame_id in tqdm(range(0, org_video_len, frame_step)):
            ...
            for i in range(INFER_LEN):
                cur_list.append(
                    torch.from_numpy(
                        transform({'image':frame_list[frame_id + i]})['image']).unsqueeze(0))
            cur_input = torch.cat(cur_list, dim=0).to(device) # Changed dim from 1 to 0
            if pre_input is not None:
                cur_input[:OVERLAP, ...] = pre_input[KEYFRAMES, ...] # Adjusted slicing for dim=0

            with torch.no_grad():
                with torch.autocast(device_type=device, enabled=(not fp32)):
                    depth = self.forward(cur_input)
                    # input shape: [T=32, 3, H, W]
                    # depth shape: [T=32, H, W]
            ...
            depth = F.interpolate(depth.unsqueeze(1),
                                  ...
                                  align_corners=True)
            depth_list += [
                depth[i][0].cpu().numpy() for i in range(depth.shape[0])
            ]
        ...
        for frame_id in range(0, len(depth_list), INFER_LEN):
            if len(depth_list_aligned) == 0:
                ...
                post_depth_list = depth_list[frame_id + align_len:frame_id +
                                             OVERLAP]
                ...
                for i in range(OVERLAP, INFER_LEN):
                    new_depth = depth_list[frame_id + i]
                    new_depth[new_depth < 0] = 0
                    depth_list_aligned.append(new_depth)

            depth_list_live.append(depth_list_aligned[-INFER_LEN:])
        depth_list = depth_list_aligned

        return np.stack(depth_list[:org_video_len],
                        axis=0), np.stack(depth_list_live[:org_video_len],
                                          axis=0)
