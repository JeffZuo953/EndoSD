...
class Dinov2ForDepth(torch.nn.Module):
    def __init__(
        ...
        self.n = (
            self.intermediate_layers[self.backbone_size]
            if decode_type == "linear4"
            else 1
        )  # self.intermediate_layers[self.backbone_size][-1]
        self.embedding_dim = self.embedding_dims[self.backbone_size]
        self.backbone_name = f"dinov2_{self.backbone_arch}"
        dinov2 = torch.hub.load(
            repo_or_dir="facebookresearch/dinov2", model=self.backbone_name
        )
        ...
        self.decode_head = decode_head_linear(
        ...
        self.sig_loss = SigLoss(
            valid_mask=True, loss_weight=1.0, warm_up=False, loss_name="loss_depth"
        )
        self.gradient_loss = GradientLoss(
            valid_mask=True, loss_weight=0.5, loss_name="loss_grad"
        )
    ...
    def forward(self, pixel_values, depth_gt=None):
        # use frozen features
        feature = self.dinov2.get_intermediate_layers(
            pixel_values, n=[2, 5, 8, 11], reshape=True, return_class_token=True
        )
        ...
        if depth_gt is not None:
            loss = self.sig_loss(
                pred.squeeze(), depth_gt.squeeze()
            ) + self.gradient_loss(pred.squeeze(), depth_gt.squeeze())
...
if __name__ == "__main__":
    ...
    print(
        "output max: ",
        output.predicted_depth.max(),
        "output min: ",
        output.predicted_depth.min(),
    )
