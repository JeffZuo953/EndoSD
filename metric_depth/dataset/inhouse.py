...
class InHouse(Dataset):
    ...
    def read_exr_depth(self, exr_path: str) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if 'V' not in channel_names:
                print(
                    f"Error: EXR file {exr_path} does not contain a 'V' channel."
                )
                return None

            data = file.channel('V', Imath.PixelType(Imath.PixelType.FLOAT))
            img_data = np.frombuffer(data, dtype=np.float32).reshape(
                size[1], size[0])
    ...
    def __getitem__(self,
                    item: int) -> Optional[Dict[str, torch.Tensor | str]]:
        img_path: str = self.filelist[item].split(" ")[0]
        depth_path: str = self.filelist[item].split(" ")[1]
