...
C3VD_TRAIN_SPLIT = "/data/c3vd/train.txt"
...
def collate_fn_pad(batch):
    ...
    for item in batch:
        image, depth, valid_mask, max_depth = item['image'], item[
            'depth'], item['valid_mask'], item['max_depth']
        h, w = image.shape[-2:]
        pad_h, pad_w = max_h - h, max_w - w

        images.append(
            F.pad(image, (0, pad_w, 0, pad_h), mode='constant', value=0))
        depths.append(
            F.pad(depth, (0, pad_w, 0, pad_h), mode='constant', value=0))
        valid_masks.append(
            F.pad(valid_mask.float(), (0, pad_w, 0, pad_h),
                  mode='constant',
                  value=0).bool())
...
def main():
    ...
    if args.dataset == "hypersim":
        trainset = Hypersim("dataset/splits/hypersim/train.txt",
                            "train",
                            size=size)
    elif args.dataset == "vkitti":
        trainset = VKITTI2("dataset/splits/vkitti2/train.txt",
                           "train",
                           size=size)
    ...
    if args.pretrained_from:
        model.load_state_dict(
            {
                k: v
                for k, v in torch.load(args.pretrained_from,
    ...
    # 使用 torch.compile 加速模型 (PyTorch 2.0+), 提供了命令行开关
        ...
        [
            {
                "params": [
                    ...
                    if "pretrained" in n and p.requires_grad
                ],
                "lr":
                args.lr
            },
    ...
    for epoch in range(args.epochs):
        ...
        for i, sample in enumerate(trainloader):
            ...
            # 修改: 不再使用每个样本的max_depth，而是使用全局的args.max_depth
            # max_depth_val = torch.tensor(sample["max_depth"],
            #                              device=img.device).view(-1, 1, 1)

            if random.random() < 0.5:
                img, depth, valid_mask = img.flip(-1), depth.flip(
                    -1), valid_mask.flip(-1)

            # 使用 autocast 上下文管理器进行前向传播
            with autocast(device_type="cuda",enabled=not args.no_amp):
                pred = model(img)
                # 修改: 在损失计算中直接使用args.max_depth
                loss = criterion(
                    pred,
                    depth,
                    (valid_mask == 1) & (depth >= args.min_depth) &
                    (depth <= args.max_depth),
                )
                # 标准化loss以进行梯度累积
                loss = loss / args.accumulation_steps

            # 使用 scaler 缩放 loss 并进行反向传播
            ...
            if rank == 0 and i % 100 == 0:
                ...
                logger.info(
                    f"Iter: {i}/{len(trainloader)}, Loss: {loss.item() * args.accumulation_steps:.3f}"
                )
        ...
        for i, sample in enumerate(valloader):
            ...
            for j in range(B):
                ...
                pred_j_resized = F.interpolate(
                    pred_j.unsqueeze(0).unsqueeze(0), (h, w),
                    mode="bilinear",
                ...
                if rank == 0 and i == 0 and j == 0:
                    ...
                    output_path = os.path.join(
                        args.save_path, filename[-2],
                        f'epoch_{epoch}_{filename[-1]}')
                    # 修改: 使用全局的args.max_depth来保存16位图像
                    pred_16bit = (pred_j_resized / current_max_depth_j *
                                  65535).to(torch.uint16)
                    cv2.imwrite(output_path, pred_16bit.cpu().numpy())

                # 修改: 使用全局的args.max_depth来生成最终的有效mask
                final_valid_mask_j = (valid_mask_j == 1) & (
                    depth_j >= args.min_depth) & (depth_j
                                                  <= current_max_depth_j)
                if final_valid_mask_j.sum() < 10:
                    continue

                cur_results = eval_depth(pred_j_resized[final_valid_mask_j],
                                         depth_j[final_valid_mask_j])
        ...
        if rank == 0:
            ...
            logger.info(
                "{:>8}, {:>8}, {:>8}, {:>8}, {:>8}, {:>8}, {:>8}, {:>8}, {:>8}"
                .format(*tuple(results.keys())))
            logger.info(
                "{:8.3f}, {:8.3f}, {:8.3f}, {:8.3f}, {:8.3f}, {:8.3f}, {:8.3f}, {:8.3f}, {:8.3f}"
                .format(*tuple([(v / nsamples).item()
            ...
            for name, metric in results.items():
                writer.add_scalar(f"eval/{name}", (metric / nsamples).item(),
                                  epoch)
        ...
        if rank == 0:
            ...
            # 由于previous_best已经在前面更新，直接使用is_best_abs_rel作为判断标准
            if is_best_abs_rel:
                torch.save(checkpoint,
                           os.path.join(args.save_path, "best_abs_rel.pth"))
