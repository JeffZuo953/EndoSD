...
def main():
    ...
    for pred_path in tqdm(pred_files, desc="正在评估"):
        ...
        try:
            ...
            gt_tensor = torch.from_numpy(gt_classes)
            
            # SegMetric 需要 B, H, W 格式, 所以我们给它加上batch维度
            metric_seg.update(gt_tensor.unsqueeze(0), pred_tensor.unsqueeze(0))
            evaluated_samples += 1
