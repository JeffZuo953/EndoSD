...
from .attention import LoRASelfAttention
from .moe import MoELayer
from .ffn_layers import Mlp_LoRA

class Block(SelfAttentionBlock):
    def __init__(
        self,
        dim: int,
        num_heads: int,
        ...
        act_layer: Callable[..., nn.Module] = nn.GELU,
        norm_layer: Callable[..., nn.Module] = nn.LayerNorm,
        attn_class: Callable[..., nn.Module] = LoRASelfAttention,
        ...
        **kwargs,
        ...
        # We need to call nn.Module's __init__ first
        nn.Module.__init__(self)

        use_lora = 'lora' in mode
        use_moe = 'moe' in mode
        
        # Then initialize components directly, bypassing SelfAttentionBlock's __init__
        self.norm1 = norm_layer(dim)
        self.attn = attn_class(
            dim,
            num_heads=num_heads,
        ...
        if use_moe:
            ...
            # Only pass LoRA parameters if LoRA is active
            if use_lora:
                self.mlp = ffn_layer(
                    in_features=dim,
                    hidden_features=mlp_hidden_dim,
