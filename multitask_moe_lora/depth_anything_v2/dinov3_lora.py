...
# 导入dinov3的LoRA/MoE层和工具
from .dinov3_layers.lora import mark_only_lora_as_trainable, lora_state_dict
from .dinov3_layers.ffn_layers import SwiGLUFFN, SwiGLUFFN_LoRA
from .dinov3_layers.moe import MoELayer as FFN_MoE_LoRA
from .dinov3_layers.attention import SelfAttention as SelfAttention_LoRA
from .dinov3_layers.block import Block # 使用LoRA/MoE Block
...
class DinoVisionTransformer_LoRA(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if ffn_layer == "swiglu":
            ffn_layer_class = SwiGLUFFN if self.attention_only_lora else SwiGLUFFN_LoRA
        else:
...
def DINOv3_LoRA(model_name, mode='original', **kwargs):
    model_zoo_lora = {
        "dinov3_vits16": dinov3_vits16_lora,
        "dinov3_vits16plus": dinov3_vits16plus_lora,
        "dinov3_vitb16": dinov3_vitb16_lora,
        "dinov3_vitl16": dinov3_vitl16_lora,
    }
    
    if model_name not in model_zoo_lora:
        raise ValueError(f"Unknown DINOv3 LoRA model name: {model_name}")
