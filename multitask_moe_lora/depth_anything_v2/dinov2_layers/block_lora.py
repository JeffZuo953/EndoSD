...
class Block_LoRA(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # Instantiate FFN layer, conditionally passing LoRA arguments
        ffn_args = {
            'in_features': dim,
        ...
        # Check if the ffn_layer is a LoRA-aware layer before passing LoRA parameters
        # A simple way is to check the class name. A more robust way might involve checking inheritance or attributes.
        if "LoRA" in ffn_layer.__name__ or (hasattr(ffn_layer, 'func') and "LoRA" in ffn_layer.func.__name__):
            ffn_args['lora_r'] = lora_r
            ffn_args['lora_alpha'] = lora_alpha
            ffn_args['lora_dropout'] = lora_dropout

        self.mlp = ffn_layer(**ffn_args)
