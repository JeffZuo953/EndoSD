...
# ==============================================================================
# Multi-Task Training Configuration Script (Depth Estimation + Semantic Segmentation) - DINOv3
# ==============================================================================

# Basic Configuration
NUM_GPUS=3
CUDA_DEVICES="3,4,5" # Specify GPU devices
ENCODER="dinov3_vits16plus"   # Model encoder: vits, vitb, vitl, vitg, dinov3_vits16, etc.
...
# Training Parameters
EPOCHS=500
BATCH_SIZE=25        # Reduced from 40 to 20 to save memory
SEG_BATCH_SIZE=25    # Reduced from 40 to 20 to save memory
VAL_BATCH_SIZE=100    # Improve validation speed, consistent with dinov2
# BATCH_SIZE=80
...
WEIGHT_DECAY=0.01
IMG_SIZE=518
FROZEN_BACKBONE="false" # For PEFT, backbone weights are frozen
# FROZEN_BACKBONE="true" # For PEFT, backbone weights are frozen
USE_MIXED_PRECISION=true # Enable mixed precision training (AMP). Note: Due to DWA numerical sensitivity, enabling this may cause training failure.
...
MODE="original" # Use new mode API
...
DATASET_CONFIG_NAME="server_hk_01"  # 'server_sz' or 'server_hk_01'
PATH_TRANSFORM_NAME="sz_to_hk"      # 'sz_to_hk' or 'none'
...
# Resume Checkpoint Path (If set, this overrides pretrained weights and restores the full training state)
RESUME_CHECKPOINT=""
...
# Set distributed environment variables (multi-GPU mode)
# export RANK=0
# export LOCAL_RANK=0
export WORLD_SIZE=${NUM_GPUS}
export MASTER_ADDR=localhost
export MASTER_PORT=20697
...
cp -r /data/ziyi/multitask/code/DepthAnythingV2/multitask_moe_lora "${SAVE_PATH}/code"
