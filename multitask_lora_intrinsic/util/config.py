...
class TrainingConfig:
    ...
    # 模式选择参数
    mode: str = "original"  # 可选择: "original", "lora-only", "moe-only", "lora-moe"
    ...
    intrinsics_loss_weight: float = 0.1
    ...
    # Endonerf dataset specific paths
    endonerf_rootpath: Optional[str] = None
    endonerf_filelist: Optional[str] = None
    # StereoMIS dataset specific paths
    stereomis_rootpath: Optional[str] = None
    stereomis_filelist: Optional[str] = None
    kidney3d_rootpath: Optional[str] = None
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--mode",
                        default="original",
                        choices=["original", "lora-only", "moe-only", "lora-moe"],
                        help="Training mode: original (no PEFT), lora-only, moe-only, or lora-moe")
    ...
    parser.add_argument("--intrinsics-loss-weight", default=0.1, type=float, help="Weight for intrinsics (fx/fy) regression loss")
    parser.add_argument("--dwa-temperature", default=2.0, type=float, help="Temperature parameter for DWA")
    
    # 数据参数
    parser.add_argument("--img-size", default=518, type=int)
    parser.add_argument("--dataset-config-name", type=str, default="server_hk_01", choices=['server_sz', 'server_hk_01', 'hamlyn', 'endonerf', 'stereomis', 'kidney3d'],
                        help="Name of the dataset configuration to use (e.g., 'server_sz', 'server_hk_01', 'hamlyn', 'endonerf', 'stereomis', 'kidney3d')")
    ...
    parser.add_argument("--endonerf-rootpath", type=str, default=None, help="Root path for Endonerf dataset")
    parser.add_argument("--endonerf-filelist", type=str, default=None, help="Filelist path for Endonerf dataset")
    parser.add_argument("--stereomis-rootpath", type=str, default=None, help="Root path for StereoMIS dataset")
    parser.add_argument("--stereomis-filelist", type=str, default=None, help="Filelist path for StereoMIS dataset")
    parser.add_argument("--kidney3d-rootpath", type=str, default=None, help="Root path for Kidney3D dataset")
...
def args_to_config(args: argparse.Namespace) -> TrainingConfig:
    ...
    return TrainingConfig(encoder=args.encoder,
                          ...
                          intrinsics_loss_weight=getattr(args, 'intrinsics_loss_weight', 0.1),
                          ...
                          endonerf_rootpath=getattr(args, 'endonerf_rootpath', None),
                          endonerf_filelist=getattr(args, 'endonerf_filelist', None),
                          stereomis_rootpath=getattr(args, 'stereomis_rootpath', None),
                          stereomis_filelist=getattr(args, 'stereomis_filelist', None),
                          kidney3d_rootpath=getattr(args, 'kidney3d_rootpath', None),
...
def validate_config(config: TrainingConfig) -> List[str]:
    ...
    # 检查seg_bs的合理性
    if getattr(config, 'intrinsics_loss_weight', 0.0) < 0:
        errors.append("intrinsics_loss_weight must be non-negative")
